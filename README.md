...
```math
\mathcal{L}_{\text{ctrl}} = \|a_t - a_t^*\|_1 = |th_t - th_t^*| + |st_t - st_t^*| + |br_t - br_t^*|
```

Where:
- **`L_wp` (Waypoint Regression):** $L_1$ trajectory displacement loss over the look-ahead horizon ($P = 4$).
- **`L_ctrl` (Actuator Command Loss):** Continuous physical control loss over steering, throttle, and brake.
- **`L_coarse` (Auxiliary Guidance):** Supervised pre-fusion alignment for the feedforward query corridor generator.
- **`L_consistency` (Kinematic Regularizer):** Second-order relative displacement smoother suppressing high-frequency steering jitter.
